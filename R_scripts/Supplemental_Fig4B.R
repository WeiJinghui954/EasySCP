...
merged_df_clean <- SCP_expression[, c("SCP_7", "SCP_8")]
spearman_cor <- cor(merged_df_clean$SCP_7, merged_df_clean$SCP_8, method = "spearman")
...
#  Save plot
PV_marker <- c("Cdh1","Cyp2f2","Gls2","Gldc","Adh1","Arg1","Pgk1","Ass1","Uba1","Acaca")
plots<-ggplot(merged_df_clean, aes(x = SCP_7, y = SCP_8)) +
  geom_point(size = 3, color = "grey80") +
  geom_point(data = subset(merged_df_clean, Gene %in% PV_marker),
             aes(x = SCP_7, y = SCP_8), color = "#A1D690", size = 4) +
  geom_text_repel(data = subset(merged_df_clean, Gene %in% PV_marker),
  ...
  labs(x = "SCP_7", y = "SCP_8") +
  annotate("text", x = 0, y = 6, label = cor_text, size = 6, hjust = 0, color = "black")

ggsave(filename = "../Output/Figure/Supplemental Figure 4B.png", plot = plots, width = 6, height = 6,limitsize = FALSE)
