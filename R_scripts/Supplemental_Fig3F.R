...
plots <- lapply(valid_genes, function(gene) {
  if (gene %in% colnames(tmp_output)) {
    ggplot(tmp_output, aes(x = x, y = y, fill = .data[[gene]])) +
      ...
      scale_fill_gradient(
        low = "#ffffff", high = "#ff8000",
        name = NULL,  # 去掉图例标题
        breaks = c(min(tmp_output[[gene]], na.rm = TRUE),
                   max(tmp_output[[gene]], na.rm = TRUE)),
        labels = c("Low", "High"),
        guide = guide_colorbar(
          barwidth = 2, barheight = 65,
          ticks = FALSE,
...
ggsave(filename = "../Output/Figure/Supplemental Fig3F.png", plot = final_plot, width = 18, height = 20,limitsize = FALSE)
